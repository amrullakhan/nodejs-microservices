import {handleProductErrors} from './HandleProductErrors';
...
describe('HandleProductErrors', () => {
    ...
    it('should return a 400 status code if product id is required', () => {
        const error = new Error(enums.ErrorsEnum.PRODUCT_ID_REQUIRED);
        const response = handleProductErrors(error);

        expect(response.statusCode).toBe(400);
        expect(response.body.message).toBe(enums.ErrorsEnum.PRODUCT_ID_REQUIRED);
    });

    it('should return a 400 status code if product name is required', () => {
        const error = new Error(enums.ErrorsEnum.PRODUCT_NAME_REQUIRED);
        const response = handleProductErrors(error);

        expect(response.statusCode).toBe(400);
        expect(response.body.message).toBe(enums.ErrorsEnum.PRODUCT_NAME_REQUIRED);
    });
